...
def evaluate(agent_type='q_learning', render=True):
    ...
    def get_danger_zones(obstacles):
        ...
        for obs_row, obs_col, dr, dc, _ in obstacles:
            ...
            for buffer_dr, buffer_dc in [(0,1), (1,0), (0,-1), (-1,0)]:
                buffer_pos = (obs_row + dr + buffer_dr, obs_col + dc + buffer_dc)
                danger_zones.add(buffer_pos)
    ...
    while not done and steps < max_steps:
        ...
        if will_obstacle_enter_agent_position(state, maze.moving_obstacles):
            print(f"Obstacle approaching agent at {state}. Must move to avoid collision.")
            ...
            for test_action in range(4):
                ...
                if (maze.is_valid_position(*test_next_state) and 
                    ...
                    goal_dist = abs(test_next_state[0] - maze.goal[0]) + abs(test_next_state[1] - maze.goal[1])
                    # Calculate safety score (distance to nearest obstacle)
                    safety_score = min(
                        abs(test_next_state[0] - obs_row) + abs(test_next_state[1] - obs_col)
                        for obs_row, obs_col, _, _, _ in maze.moving_obstacles
                    ) if maze.moving_obstacles else 10
                    
                    safe_moves.append((test_action, test_next_state, goal_dist, safety_score))
            ...
            if safe_moves:
                ...
                print(f"Emergency move to {next_state} to avoid incoming obstacle")
                ...
                print(f"WARNING: No safe moves available to avoid incoming obstacle at {state}")
        ...
        if agent_type == 'policy_gradient':
            action = agent.get_action(state, training=False, goal=maze.goal, maze=maze)
        else:
        ...
        if collision:
            print(f"Predicted collision or danger at {next_state}. Finding alternative path.")
            ...
            for alt_action in possible_actions:
                ...
                if (maze.is_valid_position(*alt_next_state) and 
                    ...
                    # Calculate distance to goal as a heuristic
                    goal_dist = abs(alt_next_state[0] - maze.goal[0]) + abs(alt_next_state[1] - maze.goal[1])
                    # Calculate safety score (distance to nearest obstacle)
                    safety_score = min(
                        abs(alt_next_state[0] - obs_row) + abs(alt_next_state[1] - obs_col)
                        for obs_row, obs_col, _, _, _ in maze.moving_obstacles
                    ) if maze.moving_obstacles else 10
                    
                    safe_moves.append((alt_action, alt_next_state, goal_dist, safety_score))
