...
def train(agent_type="q_learning"):
    ...
    maze = Maze(width=10, height=10,
                num_moving_obstacles=num_obstacles, use_seed=None)
        ...
        while not done and steps < 300:
            ...
            if agent_type == "policy_gradient":
                action = agent.get_action(
                    state, training=True, goal=maze.goal, maze=maze)
            else:
            ...
            if not maze.is_valid_position(*next_state) or next_state in current_obstacle_positions or next_state in predicted_obstacle_positions or path_crossing:
                ...
                current_dist = abs(
                    row - maze.goal[0]) + abs(col - maze.goal[1])
                new_dist = abs(next_state[0] - maze.goal[0]) + \
                    abs(next_state[1] - maze.goal[1])
    ...
    np.save(os.path.join(results_dir, 'exploration_history.npy'),
            np.array(agent.exploration_history))
    ...
    plt.suptitle(
        f'Training Results for {agent_type.replace("_", " ").title()} Agent')
...
if __name__ == "__main__":
    ...
    if args.agent:
        ...
        print("Please select an agent type to train:")
        print("1. Q-Learning")
        print("2. Policy Gradient")
        choice = input("Enter 1 or 2: ")
        
        if choice == "1":
            train(agent_type="q_learning")
        elif choice == "2":
            train(agent_type="policy_gradient")
        else:
